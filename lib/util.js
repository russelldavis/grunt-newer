  ...
  async.map(files, function(obj, done) {
    ...
    if (obj.dest) {
      fs.stat(obj.dest, function(err, stats) {
        ...
        var srcAndDeps = obj.src.concat(deps);
        return anyNewer(srcAndDeps, stats.mtime, function(err, any) {
          done(err, any && obj);
        });
      });
    } else {
      anyNewer(deps, previous, function(err, any) {
        if (err) {
          return done(err);
        }
        if (any) {
          // dependencies changed, use all src files
          return done(null, obj);
        }
